...
>
    ...
    <main class="flex-1 overflow-y-auto px-6 pb-4 pt-2">
        ...
        <div class="relative aspect-[353/207] w-full overflow-hidden rounded-[10px] bg-[#D9D9D9]">
            @if ($image)
                <img
                    src="{{ $image }}"
                    alt="{{ $name }}"
            ...
            {{-- Prev / next (decorative for now) --}}
            <button
                type="button"
                class="absolute right-2 top-1/2 flex size-7 -translate-y-1/2 items-center justify-center text-white"
                aria-label="الصورة السابقة"
            >
                <svg class="size-5 drop-shadow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                    <path d="M9 6l6 6-6 6"/>
                </svg>
            </button>
            <button
                type="button"
                class="absolute left-2 top-1/2 flex size-7 -translate-y-1/2 items-center justify-center text-white"
        ...
        {{-- Title + price --}}
        <h2 class="mt-4 text-right text-2xl font-bold text-[#D41D38]">{{ $name }}</h2>
        <p class="mt-1 text-right text-xl font-bold text-black">{{ $price }} جنيه</p>
        ...
        <button
            ...
            class="mt-4 flex h-[52px] w-full items-center justify-center gap-2 rounded-[20px] bg-[#D81D35] text-base font-bold text-white shadow-[0px_4px_7.5px_0px_rgba(0,0,0,0.25)] transition active:scale-[0.99]"
        >
            <span>أضف لبوكس الهدايا</span>
        ...
        {{-- Description --}}
        <p class="mt-3 text-justify text-sm font-medium leading-[25px] text-black">
            {{ $description }}
        </p>

        {{-- Related products --}}
        <div class="-mx-6 mt-6 flex gap-2 overflow-x-auto px-6 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            @foreach ($related as $product)
                <div class="w-[133px] shrink-0">
                    <div class="relative h-[159px] w-full overflow-hidden rounded-[10px] bg-[#D9D9D9]">
                        @if ($product['image'])
                            <img
                                src="{{ $product['image'] }}"
                                alt="{{ $product['name'] }}"
                        ...
                        <button
                            type="button"
                            @if ($product['id']) wire:click="addRelated({{ $product['id'] }})" @endif
                            class="absolute bottom-2 left-2 z-10 flex size-7 items-center justify-center rounded-full bg-[#BDBDBD] text-[#757575] transition active:scale-95 active:bg-[#D81D35] active:text-white"
