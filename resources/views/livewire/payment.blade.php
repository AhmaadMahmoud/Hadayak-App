...
>
    ...
    {{-- Scrollable body --}}
    <main class="flex-1 overflow-y-auto px-5 pb-4 pt-1">
        {{-- Payment methods --}}
        <section>
            <h2 class="px-1 text-right text-sm font-bold text-[#5C403C]">وسيلة الدفع</h2>
            ...
            <div class="mt-3 space-y-3">
                @foreach ($methods as $method)
                    <button
                        ...
                        @class([
                            'flex w-full items-center justify-between rounded-[12px] p-4 text-right transition',
                            'border border-[#B70011] bg-[#FFF0EE]' => $selected === $method['id'],
                            'border border-[#F3EFE7] bg-white' => $selected !== $method['id'],
                            'opacity-40 grayscale' => $method['disabled'],
                        ])
                        aria-pressed="{{ $selected === $method['id'] ? 'true' : 'false' }}"
                    >
                        <span class="flex items-center gap-3">
                            ...
                            <span
                                @class([
                                    ...
                                    'bg-[#B70011]' => $selected === $method['id'],
                                    'border-2 border-[#E6BDB8]' => $selected !== $method['id'],
                                ])
                            >
                                @if ($selected === $method['id'])
                                    <span class="size-2 rounded-[4px] bg-white"></span>
                                @endif
                            </span>

                            <span class="flex flex-col items-start">
                                <span class="text-base font-medium text-[#281715]">{{ $method['label'] }}</span>
                                @if ($method['note'])
                                    <span class="text-[10px] text-[#BA1A1A]">{{ $method['note'] }}</span>
                                @endif
                            </span>
                        </span>

                        {{-- Method icon (left in RTL) --}}
                        <span class="text-[#5C403C] opacity-60">
                            @if ($method['id'] === 'card')
                                <svg class="h-4 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true">
                            ...
                            @elseif ($method['id'] === 'instapay')
                                <svg class="h-4 w-[22px]" viewBox="0 0 24 18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true">
                                    <rect x="1" y="1" width="22" height="16" rx="2"/>
                                    <circle cx="12" cy="9" r="4"/>
                                </svg>
                            @else
                                <svg class="h-4 w-[22px]" viewBox="0 0 24 20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    <path d="M1 3h13v11H1zM14 7h5l4 4v3h-9z"/>
        ...
        {{-- Important note --}}
        <div class="mt-4 flex items-start gap-3 rounded-[12px] border border-[#B70011]/10 bg-[#FFF0EE] p-4">
            <svg class="mt-0.5 size-5 shrink-0 text-[#B70011]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true">
                <circle cx="12" cy="12" r="10"/>
                <path d="M12 8h.01M12 12v4"/>
            </svg>
            <p class="text-right text-sm leading-relaxed text-[#5C403C]">
                لا تقلق، في حالة ارسال الهدية لشخص اخر لن يتم ارسال فاتورة السعر 😉
            </p>
        </div>
        ...
        <section class="mt-6 px-1">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-bold text-[#281715]">ملخص الطلب</h2>
                <a href="{{ route('gift-box') }}" wire:navigate class="text-xs font-medium text-[#B70011] underline">تعديل</a>
            </div>

            {{-- Items --}}
            <div class="mt-2 space-y-2 border-b border-[#F3EFE7] pb-3 pt-2">
                @foreach ($items as $item)
                    <div class="flex items-center justify-between gap-3">
                        <p class="min-w-0 flex-1 truncate text-right text-sm text-[#281715]">
                            {{ $item['name'] }}
                            <span class="text-xs font-medium text-[#585F6C]">× {{ $item['qty'] }}</span>
                        </p>
                        <p class="shrink-0 text-sm font-bold text-[#B70011]">{{ number_format($item['price'] * $item['qty']) }} ج.م</p>
                    </div>
                @endforeach

                @if ($wrapPrice > 0)
                    <div class="flex items-center justify-between text-sm text-[#585F6C]">
                        <span>🎀 تغليف الهدية</span>
                        <span>{{ number_format($wrapPrice) }} ج.م</span>
                    </div>
                @endif

                @if ($cardPrice > 0)
                    <div class="flex items-center justify-between text-sm text-[#585F6C]">
                        <span>💌 بطاقة المعايدة</span>
                        <span>{{ number_format($cardPrice) }} ج.م</span>
                    </div>
                @endif
            </div>
            ...
            <div class="mt-3 space-y-2">
                <div class="flex items-center justify-between text-sm text-[#585F6C]">
                    <span>{{ number_format($itemsTotal) }} ج.م</span>
                    <span>قيمة المنتجات</span>
                </div>
                <div class="flex items-center justify-between text-sm text-[#585F6C]">
                    <span>{{ number_format($delivery) }} ج.م</span>
                    <span>التوصيل</span>
                </div>
                <div class="flex items-center justify-between pt-2">
                    <span class="text-lg font-bold text-[#B70011]">{{ number_format($this->total) }} ج.م</span>
                    <span class="text-sm font-bold text-[#281715]">الإجمالي النهائي</span>
                </div>
            </div>
        </section>
    </main>
    ...
    <div class="shrink-0 px-5 pb-3 pt-2 shadow-[0px_-5px_9.2px_0px_rgba(0,0,0,0.10)]">
        @if ($error)
            <p class="mb-2 text-center text-sm font-bold text-[#D81D35]">{{ $error }}</p>
        @endif
        ...
        >
            <span wire:loading.remove>اتمام الطلب</span>
            <span wire:loading>جاري إرسال الطلب...</span>
        </button>
    </div>
